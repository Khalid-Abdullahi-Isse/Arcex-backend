...
].map(([key, phone, email, name, region, verified]) => ({
    key,
    phone,
    email,
    name,
...
function l(title, description, region, district, sizeSqm, price, sellerKey, latitude, longitude, createdDaysAgo, photoCount) {
    return {
        title,
        description,
        region,
...
function requireDevDatabase() {
    ...
    if (!safeHost || !safeName || !safeUser) {
        throw new Error(`Refusing to reset database "${dbName}" on "${host}" as "${user}". Use a local AcreX dev database.`);
    }
}
...
function documentTypes(index) {
    const sets = [
        [client_1.ListingDocumentType.TITLE_DEED, client_1.ListingDocumentType.ID_CARD, client_1.ListingDocumentType.SURVEY_MAP],
        [client_1.ListingDocumentType.TITLE_DEED, client_1.ListingDocumentType.ID_CARD],
        [client_1.ListingDocumentType.TITLE_DEED, client_1.ListingDocumentType.SURVEY_MAP],
        [client_1.ListingDocumentType.TITLE_DEED, client_1.ListingDocumentType.ID_CARD, client_1.ListingDocumentType.OTHER],
    ];
...
async function main() {
    ...
    const docLabels = {
        [client_1.ListingDocumentType.TITLE_DEED]: 'Title Deed',
        [client_1.ListingDocumentType.ID_CARD]: 'ID Card',
        [client_1.ListingDocumentType.SURVEY_MAP]: 'Survey Map',
        [client_1.ListingDocumentType.OTHER]: 'Supporting Document',
    };
    for (let index = 0; index < listingSeeds.length; index++) {
        const seed = listingSeeds[index];
        const seller = usersByKey.get(seed.sellerKey);
        if (!seller)
            throw new Error(`Seller "${seed.sellerKey}" was not created.`);
        const reviewed = seed.status !== client_1.ListingStatus.PENDING_REVIEW;
